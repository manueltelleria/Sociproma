...
#Se muestran los datos asociados al id en tratamiento
      ...
      $resultado = $miDoctor->consulta($miconexion->Conexion_ID, $Where);
...
#Se hace la inserciòn de los valores de la pantalla
  ...
  verconsulta( $smarty, $miDoctor->consulta($miconexion->Conexion_ID, $Where));
...
function buscar( $smarty, $Conexion_ID, $Doctor ) {
  ...
  $resultado = $Doctor->consulta($Conexion_ID, join(" AND ", $Where));
// mostrarmos los registros
  ...
  while ($row = $resultado->fetch_assoc()) {
    ...
    $Datos['stelefono']         = $row['stelefono'];
    $Datos['stelefono_1']       = $row['stelefono_1'];
    $Datos['desc_especialidad'] = $row['desc_especialidad'];
    $Datos['clase']             = $clase;
